...
public class Main01 {
	public static void main(String[] args) {
		...
		System.out.println("str1_last_pos : " + str2_last_pos);
		System.out.println("--------------------");
		...
		System.out.println("--------------------");
		/*
		 * 문자열은 ==로 비교가 안된다 이유는 문자는 객체이기 때문에 문자열 비교가 아니라 주솟값을 비교하게 되버린다.
		 */
		...
		/*
		 * 8) 문자열을 자르기 - 첫 번째 파라미터 : 시작위치 - 두 번째 파라미터 : 끝 위치
		 */
		String sub_str1 = str1.substring(0, 2);// 자바
		// 두 번째 파라미터가 없는경우 끝까지 자른다.( 오버플로우)
		...
		/*
		 * 9)형식에 따른 문자열 출력을 위한 Static 메서드 사용 -%d : 정수 -%s : 문자 -%f : float형 소수 -%3d :
		 * 숫자를 3자리로 맞춰서 출력. 자리수가 맞지 않으면, 앞에 공백 출력함 -%03d : 자릿수가 맞지 않으면 0을 출력 -%3s : 문자열을
		 * 3자리로 맞춰서 출력. 자릿수가 맞지 않으면 앞에 공백 출력 (%03s와 같은 형식은 사용 불가 자바는 뒤의 갯수가 다르면 혹은 형이 맞지
		 * 않으면 무조건 에러가 난다. 무. 조.건
		 */
		int yy = 2014;
		int mm = 3;
		int dd = 14;
